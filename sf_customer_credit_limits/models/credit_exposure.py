...
class CreditExposure(models.Model):
    ...
    rule_id = fields.Many2one(comodel_name='sf.customer.credit.limits.credit.limit.rule', ondelete='restrict')
    ...
    action_taken = fields.Selection([
        ('none', 'None'), ('warned', 'Customer Warned'),
        ('throttled', 'Orders Throttled'), ('blocked', 'Orders Blocked'),
        ('sent_to_collections', 'Sent to Collections'),
        ], string='Action Taken', default='none', copy=False)
