# -*- coding: utf-8 -*-
"""Quality Alert Escalation Matrix models"""
from odoo import api, fields, models, _
from odoo.exceptions import UserError


class SfQuality_alert_escalation(models.Model):
    _name = 'sf.quality_alert_escalation'
    _description = 'Quality Alert Escalation Matrix'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = 'create_date desc, id desc'

    name = fields.Char(string='Reference', required=True, copy=False, readonly=True, default='New')
    company_id = fields.Many2one('res.company', string='Company', required=True, default=lambda s: s.env.company, tracking=True)
    alert_type = fields.Char(string='Alert Type', required=True)
    severity = fields.Selection([
        ('critical', 'Critical'),
        ('major', 'Major'),
        ('minor', 'Minor'),
        ], string='Severity', required=True)
    escalation_level = fields.Integer(string='Level', required=True)
        ...
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'sf.quality_alert_escalation') or 'NEW'
...
class _Boost(models.Model):
    _inherit = 'sf.quality_alert_escalation'
